...
import com.gesoft.service.AActivityService;
...
public class AActivityController extends BaseController
{
    ...
    @Resource
    private AActivityService aActivityService;
    ...
    public @ResponseBody MsgModel search(ServiceModel model)
    {
        ...
        try
        {
            aActivityService.findPageList(model, msgModel);
        }
        catch (Exception e)
        {
    ...
    public @ResponseBody MsgModel add(ActivityModel model)
    {
        MsgModel msgModel = new MsgModel();
        try
        {
            ...
            if (aActivityService.save(model) > 0)
            {
    ...
    public @ResponseBody MsgModel modify(ActivityModel model)
    {
        MsgModel msgModel = new MsgModel();
        try
        {
            if (aActivityService.update(model) > 0)
            {
                msgModel.setSuccess(GLOBAL_MSG_BOOL_SUCCESS);
            }
        }
    ...
    public @ResponseBody MsgModel delete(ActivityModel model)
    {
        MsgModel msgModel = new MsgModel();
        try
        {
            if (aActivityService.delete(model) > 0)
            {
                msgModel.setSuccess(GLOBAL_MSG_BOOL_SUCCESS);
            }
        }
    ...
    public @ResponseBody List<DoctorModel> querySimpleDoctorInfo(QueryModel model)
    {
        ...
        try
        {
            list = aActivityService.queryDoctors(model);
        }
        catch (Exception e)
        {
    ...
    public @ResponseBody long[] queryAssociatedDoctorIds(QueryModel model)
    {
        ...
        try
        {
            List<DoctorModel> list = aActivityService.queryAssociatedDoctorIds(model);
            if (list != null) {
    ...
    public @ResponseBody MsgModel associateDoctor(QueryModel model)
    {
        ...
        try
        {
            msgModel.setSuccess(aActivityService.insertDoctorAssociation(model));
