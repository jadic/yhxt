...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
	...
	<script type="text/JavaScript">
		...
		function goMerge(id)
		{
			window.location.href = "<c:url value='/n/search/mergeService.do'/>?id="+id;
		}
						...
						<c:if test="${not empty serviceFlys }">
							<c:forEach items="${serviceFlys }" var="serviceItem" varStatus="item">
								<tr class='<c:if test="${item.index mod 2 == 0 }">abnormal odd</c:if><c:if test="${item.index mod 2 == 1 }">even</c:if>' style="height: 40px;">
									<td><a href="javascript:void(0)" onclick="funNurseDetail(${serviceItem.id})">${serviceItem.name }</a></td>
									<td>${serviceItem.sdate }</td>
									<td>${serviceItem.edate }</td>
									<td>
										<a href="javascript:void(0)" onclick="goMerge(${serviceItem.id})"><img src="<c:url value='/patient/themes/images/phone_editor.png'/>">编辑</a>
