				...
				success:function(data)
				{
					...
					if(data.success)
					{
						$.messager.confirm('确认', "操作成功，你要返回查询列表吗？", function(r)
						{
							if (r)
							{
								window.location.href = "<c:url value='/n/search/activity.do'/>";
							}
						});
					}
					else
	...
	<div class="account">
		<div class="account_title">
	      <ul>
	        ...
	        <li class="account_titleGray">当前位置：<c:if test="${query.id == 0}">增加活动信息</c:if><c:if test="${query.id > 0}">修改活动信息</c:if></li>
	      </ul>
	    </div>
	</div>    
	        ...
	        <div class="informationModify_main">
	        	<table cellpadding="0" border="0" cellspacing="0" style="font-size: 13px; width: 100%;">
		    		<tr>
		    			...
		    			<td align="left">
		    				<input class="inputMin_informationModify text-input" type="text" value="${activity.name }" id="name" maxlength="16" style="width: 400px;">
		    			</td>
		    			<td rowspan="5" align="center">
		    				<table>
		    					<tr>
		    						<td align="center">
		    							<c:if test="${empty activity.icon }">
		    							<img style="width:120px; height: 140px;" id="header_photo" src="<c:url value='/nurse/themes/images/default_head.gif'/>">
		    							</c:if>
		    							<c:if test="${not empty activity.icon }">
		    							<img style="width:120px; height: 140px;" id="header_photo" src="<c:url value='/'/>${activity.icon}">
		    							</c:if>
		    						</td>
		    					</tr>
		    					<tr>	
		    						<td class="thead_informationModify" align="center">
		    							<input type="hidden" id="icon" value="${activity.icon}"/>
		    		...
		    		<tr>
		    			...
		    			<td align="left">
		    				<input class="inputMin_informationModify text-input Wdate" value="${activity.sdate }" id="sdate" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',readOnly:true})" type="text" value="${nurseRequest.userName }" maxlength="16" style="width: 400px;">
		    			</td>
		    		</tr>
		    		<tr>
		    			<td align="right" style="padding: 0px 5px 5px 0px; width:70px; height: 30px; color: #aeaeae; font-size: 13px;">
		    			...
		    			<td align="left">
		    				<input class="inputMin_informationModify text-input Wdate" value="${activity.edate }" id="edate" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',readOnly:true})" type="text" value="${nurseRequest.userName }" maxlength="16" style="width: 400px;">
		    			</td>
		    		</tr>
		    		<tr>
		    			<td align="right" style="padding: 0px 5px 5px 0px; width:70px; height: 30px; color: #aeaeae; font-size: 13px;">
		    			...
		    			<td align="left">
		    				<input type="hidden" id="id" value="<c:if test="${empty activity }">0</c:if>${activity.id }"/>
		    				<input class="inputMin_informationModify text-input" type="text"  value="${activity.memo }" id="memo" maxlength="16" style="width: 400px;">
		    			</td>
		    		</tr>
		    		<tr>
		    			<td align="right" style="padding: 0px 5px 5px 0px; width:70px; height: 30px; color: #aeaeae; font-size: 13px;">
		    		...
		    		<tr>
		    			...
		    			<td align="left" colspan="2">
		    				<textarea id="content" style="width:  550px; height: 500px;"  class="inputMax_informationModify text-input">${activity.content }</textarea>
		    			</td>
		    		</tr>
		    		<tr>
