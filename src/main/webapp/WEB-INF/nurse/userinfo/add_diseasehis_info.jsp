  ...
  <head>
	...
	<script type="text/JavaScript">
		function funSaveInfo()
		{
			...
			$.ajax({
				url : _ctx_ + '/n/search/<c:if test="${query.id  > 0}">modifyDiseaseHis.do</c:if><c:if test="${query.id  == 0}">addDiseaseHis.do</c:if>?a='+ Math.random(),
				type : 'post',
				dataType : 'json',
				data : 
				{
				...
				success:function(data)
				{
					...
					{
						alert("操作成功！");
						window.location.href = "<c:url value='/n/search/diseasehis.do'/>?userId=${query.userId}";
					}
					else
					{
