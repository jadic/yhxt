var PageService = 
{
	...
	showGrid : function()
	{
		$("#div_grid").datagrid({
			...
			nowrap: true,
			stripe:true,
			url: _ctx_ + "/a/service/query.do",
			pageNumber: 1,
			queryParams:
			{
				"type":PageService.mServiceType,
				"name":$("#out01").val()
			},
			columns:[[
			...
			toolbar:[{
				...
				iconCls:'icon-add',
				handler:function(){
					PageMain.funCreateWinInfo("#div_win", "baseinfo/add_service_info", {param1: "add"});
				...
				handler:function(){
					...
					{
						PageMain.funCreateWinInfo("#div_win", "baseinfo/add_service_info", {param1:"modify"});
	...
	funSaveInfo : function(_param)
	{
		...
		$.ajax({
			url : _ctx_ + "/a/service/"+_param+".do?a="+ Math.random(),
			type : 'post',
			dataType : 'json',
			data : 
			{
	...
	funDelInfo : function()
	{
		...
		if(PageService.mSelDataGrid != null)
		{
			$.messager.confirm('确认', "您确认要删除选中的：【<span style='color:red; font:bold;'>"+PageService.mSelDataGrid.name+"</span>】这条记录吗？", function(r)
			{
				if (r)
				{
					/**打开进度条**/
					...
					$.ajax({
						url : _ctx_ + "/a/service/del.do?a="+ Math.random(),
						type : 'post',
						dataType : 'json',
						data : 
						{
							"id": PageService.mSelDataGrid.id						
						},
						error:function(data)
						{
							/**关闭进度条**/
						...
						success:function(data)
						{
							...
							if(data.success)
							{
								$('#div_grid').datagrid('deleteRow', $('#div_grid').datagrid('getRowIndex', PageService.mSelDataGrid));
								$.messager.alert('信息提示', data.msg, 'info');
							}
							else
							{
