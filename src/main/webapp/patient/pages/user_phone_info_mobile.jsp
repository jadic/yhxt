...
<!--
	$(function(){
		PageMain.funSetWinMaxAttr(300, 145, "上传照片", "icon-edit");
		$('#div_win').window({
			modal: true,
			title : PageMain.mWinTitle,
			shadow: false,
			top: 100,
			left: ($(window).width() - 300) > 21?($(window).width() - 300)/2:10,
			width: 300,
    			...
    			<tr>
    				<tr>
   						<td style="padding-left: 10px; font-size: 14px; height: 60px;" align="left" valign="middle">
   							图像：
	   						<input id="file" type="file" class="inputMin_informationModify text-input" style="width: 180px;" onblur="$(this).val($.trim($(this).val()))"/>
